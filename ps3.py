...
def problem3() -> SHAttered:
    ...
    * compute hash by downloading the pdfs (e.g. with requests library)
    ...
    False
    """
    #reference: https://github.com/123ojp/107SecureCodingTA/blob/7e4bde3a6ad15c4fe854135e8e5cae268dc96ede/practice_CTF_quz/ans/sha1.py
    data = {'blue_pdf': requests.get("https://shattered.io/static/shattered-1.pdf").content,
    'red_pdf': requests.get("https://shattered.io/static/shattered-2.pdf").content} 

    SHAttered = {'blue_pdf_sha1': hashlib.sha1((data['blue_pdf'])).digest(),
    'blue_pdf_sha256': hashlib.sha256((data['blue_pdf'])).digest(),
    'red_pdf_sha1': hashlib.sha1((data['red_pdf'])).digest(),
    'red_pdf_sha256': hashlib.sha256((data['red_pdf'])).digest()
    }
    return SHAttered
...
def problem4(length: int, hash: bytes, suffix: bytes) -> bytes:
    ...
    * new suffix to extend the hash with
    ...
    True
    """
    generated_hash = sha256.sha256()
    generated_hash.state = (hash, length + len(sha256_padding(length)))
    generated_hash.update(suffix)
    return generated_hash.digest()
    #reference: miroslavs comments in slack and in code above
...
def problem5(key: bytes, data: bytes) -> bytes:
    ...
    #reference:https://www.gauravvjn.com/generate-hmac-sha256-signature-in-python/