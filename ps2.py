...
def problem3(g: int, p: int) -> DHKeyPair:
    a = secrets.choice(range(0,256))
    DHKeyPair = {'a': a, 'A': (g**a) % p} 
    return DHKeyPair
...
def problem4(g: int, p: int, a: int, A: int) -> bool:
    if g > p or a < 2  or a < g or a > p or A != (g ** a) % p or A > p:
        return False
    else: 
        return True
...
def problem5(g: int, p: int, B: int, b: typing.Optional[int] = None) -> DHNegotiatedSecret:
    ...
    DHNegotiatedSecret = {'A': A,'s':shared_secret} 
    return DHNegotiatedSecret
